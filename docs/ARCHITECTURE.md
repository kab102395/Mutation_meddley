...
Version 0.6.2 uses that field as a small state envelope:
...
Version 0.6.2 keeps `Popup.ShowOptionList` for both path selection and mutation-specific stance changes.
...
Version 0.6.2 has four connected layers of content:
...
- `Carapace Evolution` is the first narrow vanilla adapter and the first full event-driven shell vertical slice
...
- the shared framework also owns the hardened bonus-damage plumbing: explicit event-object lookup, explicit damage dispatch, one shared dispatch-depth recursion guard across Mutation Meddley mutations, and dev-only tracing that distinguishes dispatch, event continuation, and observed HP loss
...
- owned mutations should treat stored meters as fuel for observable verbs, not as the final reward themselves
- hidden discovery remains mutation-local and rank-gated; no retroactive rank-9 respec flow exists in `0.6.2`
