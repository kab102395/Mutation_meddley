...
Version `0.5.2` is the current stabilization milestone on top of the strange-ecology deepening pass. It contains:
...
Existing pre-envelope saves that only store semicolon-separated evolution IDs should still load cleanly; version `0.5.2` reads that older shape and rewrites it into the current envelope on the next state change.
